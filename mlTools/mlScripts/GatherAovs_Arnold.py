...
def buildGrp(n,rebuild):
    ...
    if seqData:
        ...
        n['knobChanged'].setValue("kn=[\"before\",\"cacheLocal\",\"premultiplied\",\"format\",\"after\",\"on_error\",\"last\",\"colorspace\",\"first\"]\nn=nuke.thisNode()\nk=nuke.thisKnob()\nif k.name() in kn:\n    for node in nuke.allNodes():\n        if k.name() in node.knobs():            node[k.name()].setValue(n[k.name()].value())")
